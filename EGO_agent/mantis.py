...
def record_data(records_data, auth_token_json):
    print('record_data')
    for record_data in records_data:
        id = record_data.get('id')
        Ipv_Scan = record_data.get('Ipv_Scan')
        LoopCustomersBool = record_data.get('LoopCustomersBool')
        OutOfScope = record_data.get('OutOfScope')
        ScanProjectByID = record_data.get('ScanProjectByID')
        ScanGroupingProject = record_data.get('ScanGroupingProject')
        ScanProjectByName = record_data.get('ScanProjectByName')
        Customer_chunk_size = record_data.get('Customer_chunk_size')
        Record_chunk_size = record_data.get('Record_chunk_size')
        Global_CoolDown = record_data.get('Global_CoolDown')
        Global_RateLimit = record_data.get('Global_RateLimit')
        Port = record_data.get('Port')
        HostAddress = record_data.get('HostAddress')
        severity = record_data.get('severity')
        scan_select_vuln = record_data.get('scan_select_vuln')
        Elavate = record_data.get('Elavate')
        ...
        if auth_token_json:
            Vuln = requests.get(url, headers=auth_token_json,verify=False)
        else:
        ...
        #matcher area things to find break down put into set for concuption for shredder
            ...
            #matcher area things to find break down put into set for concuption for shredder
                ...
                if Record_chunk_size == 0:
                    ...
                    id_list= [i for i in SCOPED_set]
                    id_chunks = list(ToolBox.splited(SCOPED_set, Record_chunk_size))
            ...
            urlhost= f"{HostAddress}:{Port}/api/customers/{ScanProjectByID}"
            if auth_token_json:
                getRecords= requests.get(urlhost, headers=auth_token_json,verify=False)
            else:
                getRecords= requests.get(urlhost,verify=False)
...
if __name__ == "__main__":
    ...
    VulnUrl = f'{HostAddress}:{Port}/api/MantisControls/'    
    headers = {"Content-type": "application/json", "Accept": "application/json"}    
    ...
    while True:
        if auth_token_json:
            ...
            record_data(records,auth_token_json)
