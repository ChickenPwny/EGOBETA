    ...
    def process_bucket(self):
        ...
        try:
            ...
            if metadata_content:
                ...
                self.results["metadata_content"] = None
        except Exception as e:
...
def update_rest_api_with_bucket(record, bucket_results, api_url, api_token):
    ...
    bucket_validations = record.get("bucket_validations", [])
    ...
    if existing:
        ...
        if response.status_code == 201:
            ...
            print(f"Failed to add bucket '{bucket_results['bucket_name']}' to the API. Response: {response.text}")
        return response.json()
...
if __name__ == '__main__':
    ...
    customerID = '65b2446b-b5a8-4b31-9f1f-6c2976660d28'
    ...
    print('loading')
    print(len(output['customerrecords']))
    output = Customer_function(output['customerrecords'])
    for record in output:
        sub = record['subDomain']
        Bucket_record = record['BucketValidation']

        Bucket_loop = [b for b in Bucket_record if b['bucket_name'] == sub]

        if Bucket_loop:
            # Find the dict with the latest 'updated' date
            latest_bucket = max(
                Bucket_loop,
                key=lambda b: datetime.strptime(b['updated'], "%Y-%m-%d")
            )
            latest_update = datetime.strptime(latest_bucket['updated'], "%Y-%m-%d").date()
            if (date.today() - latest_update).days < 90:
                continue  # Skip if last update was within 90 days
        
        if '*' in sub or record['aws_scan']:
            continue  # Skip already scanned or wildcard domains
        aws_scan = record.get('aws_scan')
        aws_scan_date = record.get('aws_scan_date')
        if aws_scan and aws_scan_date:
            try:
                scan_date = datetime.strptime(aws_scan_date, "%Y-%m-%d").date()
                if (date.today() - scan_date).days < 90:
                    continue  # Skip if scanned within last 90 days
            except Exception:
                pass
        # Run S3 checks
